...
result = df.dropna() # dropna'nın droptan farkı satırda veya sütunlarda NaN değerleri varsa onları siler. varsayılanı axis=0 => satıra
result = df.dropna(axis = 1)
result = df.dropna(how = "any") # herhangi bi satırda NaN varsa o satırı getirmez
result = df.dropna(how = "all") # herhangi bi satırın ancak ve ancak tamamında NaN varsa sadece o satırı getirmez 
...
result = df.fillna(value = 'no input') # NaN olan alanları fillna metodu doldursun.
result = df.fillna(value = '1')

result = df.sum()
result = df.sum().sum() # df deki tüm elemanları toplar
